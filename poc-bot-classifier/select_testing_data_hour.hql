...
from wmf.webrequest
where year=2018 and month=10 and day=08 and hour=12
and (is_pageview=1 
...
drop table classifier_data_one_hour_sorted;

create table 
    classifier_data_one_hour_sorted 
  --  partitioned by (year, month, day)
as 
    select * from classifier_data_one_hour order by sessionId,ts limit 1000000000;

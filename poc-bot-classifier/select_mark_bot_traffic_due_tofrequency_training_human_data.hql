use nuria;
-- run testing code on session tryouts before we attemp to do it in all sessions
...
drop table if exists classifier_testing_data_human_model_result;

create table
    classifier_testing_data_human_model_result
as
select
    sessionId,
...
from
    nuria.classifier_testing_data_human_sorted st
group by 
    st.sessionId, agent_type, user_agent;


-- group by label, ideally most everything is labelled as bot 
--select count(*), label from classifier_training_human_data_model_result group by label;

-- now join to see true positives (they have to be calculated in requests, not sessions)
-- select count(*), label from classifier_negative_human_data_sorted s right join classifier_training_bot_human_model_result l on (s.sessionId=l.sessionid) group by label;

...
def main():
    ...
    def is_adjacent(data, x, y, max_x, max_y):
        adjacent = False
        candidates = []
        # north
        if y > 0:
            candidates.append(data[y-1][x])
        if x > 0:
            candidates.append(data[y][x-1])
        if y <max_y:
            candidates.append(data[y+1][x])
        if x <max_x:
            candidates.append(data[y][x+1])
        ...
        # bottom and before
        if x > 0 and y < max_y:
            candidates.append(data[y+1][x-1])
        # upand before
        if x> 0 and y > 0:
            candidates.append(data[y-1][x-1])
    ...
    for k in numbers.keys():
        ...
        for x in range(x, x +len(n)):
            adjacent = is_adjacent(data,x,y, max_x, max_y)
            if adjacent:
        ...
        if adjacent and n not in parts:
            ...
            print ("not adjacent : {0} {1}".format(n, (x,y)))
